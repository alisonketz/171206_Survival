...
for(i in 1:n.cap){
    ...
    for(j in 1:n.mort){
        if(d.cap$lowTag[i]==d.mort.ad$lowtag[j]){
            ...
            N.temp[i,3] = d.mort.ad$collarFoundDis[j]#s
            N.temp[i,4] = 0
        }
        if(N.temp[i,4]==0){
            if(is.na(N.temp[i,2])){
            ...
            if(is.na(N.temp[i,3])){N.temp[i,3]=non.harvest.survival.end}#set to the max week, because that's the following week of last known alive
...
#remove morts that occured during harvest
...
for(i in 1:n.temp){
    if(N.temp[i,4]==1){
        N.data.fit[indx,] = c(N.temp[i,1:2],1,N.temp[i,5:6])
        indx=indx+1
    }
    if(N.temp[i,4]==0){
        N.data.fit[indx,] = c(N.temp[i,1:2],1,N.temp[i,5:6])
        indx=indx+1
        N.data.fit[indx,] = c(N.temp[i,2:3],0,N.temp[i,5:6])
        indx=indx+1
    }
}
...
### Adding covariates i.e. CWD status at capture
###
######################################################################################################################3
